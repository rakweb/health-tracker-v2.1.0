...
const METRICS = [
  { key: 'HBodyBat', label: 'High Body Battery', unit: '', type: 'number' },
  { key: 'LBodyBat', label: 'Low Body Battery', unit: '', type: 'number' },
...
const DEFAULT_THRESHOLDS = {
  HBodyBattery: { warnLow: 15, bandLow: 20, bandHigh: 100, warnHigh: 999 },
  LBodyBattery: { warnLow: 15, bandLow: 20, bandHigh: 100, warnHigh: 999 },
...
const UI = {
  ...
  initChartMetricSelector() {
    ...
    METRICS.forEach(m => {
      if (m.type === 'number') {
        ...
        if (['sys', 'dia', 'spo2', 'hr', 'weightLbs', 'glucose'].includes(m.key)) opt.selected = true;
        sel.appendChild(opt);
      }
    });
  },
  ...
  refreshRiskChips(latest) {
    ...
    assess('HBodyBat', latest.HBodyBat, 'High Body Battery');
	assess('LBodyBat', latest.LBodyBat, 'Low Body Battery');
  ...
  openEntry(id = null) {
    ...
    if (id != null) {
      ...
      map('f_HBodyBat', en.HBodyBat);
	  map('f_LBodyBat', en.LBodyBat);
      ...
      ['HBodyBat', 'LBodyBat', 'stress', 'weightLbs', 'heightIn', 'waistIn', 'tempF', 'lungFluidCc', 'sys', 'dia', 'spo2', 'hr', 'resp', 'sleep', 'steps', 'glucose']
        .forEach(k => {
  ...
  openThresholds() {
    ...
    const keys = ['HBodyBat', 'LBodyBat', 'sys', 'dia', 'spo2', 'hr', 'tempF', 'stress', 'lungFluidCc', 'resp', 'sleep', 'steps', 'glucose', 'pain', 'symptoms', 'bmi', 'whtr'];
...
function normalizeHeader(h) {
  ...
  const map = {
    ...
    'HBodyBat': 'HBodyBat', 'LBodyBat': 'LBodyBat', 'stress': 'stress',
...
function csvRowToEntry(row, headerIndex) {
  const e = {
    date: null, time: null,
    HBodyBat: null, LBodyBat: null, stress: null,
	...
	glucose: null, pain: null, symptoms: null,
  ...
  const numericKeys = ['HBodyBat', 'LBodyBat', 'stress', 'weightLbs', 'heightIn', 'waistIn', 'tempF', 'lungFluidCc', 'sys', 'dia', 'spo2', 'hr', 'resp', 'sleep', 'steps', 'pain', 'symptoms', 'glucose'];
...
const Actions = {
  ...
  async saveEntryFromForm() {
    ...
    const en = {
      date: normDate,
      time: normTime,
      HBodyBat: num('f_HBodyBat'),
      LBodyBat: num('f_LBodyBat'),
      stress: num('f_stress'),
  ...
  async saveThresholds() {
    try {
      ...
      const editKeys = ['HBodyBat', 'LBodyBat', 'sys', 'dia', 'spo2', 'hr', 'tempF', 'stress', 'lungFluidCc', 'resp', 'sleep', 'steps', 'glucose', 'pain', 'symptoms', 'bmi', 'whtr'];
